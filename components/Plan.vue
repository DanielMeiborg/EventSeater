<template>
    <div>
        <div class="flex flex-col justify-between items-center w-full mb-5 md:flex-row">
            <h2 class="text-3xl font-bold">Ergebnisse</h2>
            <p v-if="bestScore !== null"><span class="badge badge-secondary">{{ -bestScore }}</span> Wünsche nicht
                erfüllt
            </p>
        </div>
            ...
            <div v-for="table in results.sort((a, b) => b[0] - a[0])"
                ...
                <div class="flex flex-col justify-center items-center">
                    <div v-for="user in table[1]" class="my-2">
                        <span class="badge" v-if="user[1]">
                            {{ user[0] }}</span>
                        <span v-else class="badge badge-error">{{ user[0] }}</span>
                    </div>
                </div>
            </div>
        </div>
